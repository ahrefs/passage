...
let get_comments_from_stdin ?(help_message = None) ?(error_prefix = "E:") () =
  ...
  | Ok () -> Lwt.return new_comments

let get_comments_from_editor ?initial ~name ?(error_prefix = "E:") () =
  let%lwt new_comments = input_and_validate_loop ~validate:validate_comments ?initial (new_text_from_editor ~name) in
  match new_comments with
  | Error e -> Shell.die "%s %s" error_prefix e
  | Ok comments -> Lwt.return (String.trim comments)
