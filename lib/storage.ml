...
(* we need to define this module here because we can't use the Devkit.Fresh module inside functors *)
...
module With_config (Config : Types.Config) = struct
  ...
  module Secret_name = Secret_name'
  ...
  module Secrets = struct
    ...
    let secret_exists_at path =
      try FilePath.add_extension Path.(project @@ abs path) ext |> Sys.file_exists
      with FilePath.NoExtension _ -> false
