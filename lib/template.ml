...
let substitute_iden node =
  ...
  | Template_ast.Iden name ->
    ...
    (try
       let plaintext = Storage.Secrets.decrypt_exn secret_name in
       let secret = Secret.Validation.parse_exn plaintext in
       Template_ast.Text secret.text
     with exn -> Devkit.Exn.fail ~exn "E: could not decrypt secret %s" (Storage.Secret_name.project secret_name))
