...
let error_not_recipient ~op_string path =
  ...
  let () =
    Base.eprintfn "E: user is not a recipient of %s. Please ask one of the following to add you as a recipient:"
      (Util.Show.show_path base_folder)
  in
  let expanded_recipients = get_expanded_recipient_names_from_folder base_folder in
  let () = List.iter (Base.eprintfn "  %s") expanded_recipients in
  Base.die "E: refusing to %s: violates invariant" op_string
...
let die_if_invariant_fails ?use_sudo ~op_string path =
  ...
  if (not (Path.is_directory full_path)) || FileUtil.ls (Path.project full_path) = [] then ()
    ...
    | true ->
      ...
      in
      if fails_invariant then Base.die "E: refusing to %s: violates invariant" op_string else ())
